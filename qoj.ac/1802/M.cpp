...
long double distancePL(const Point<T> &p, const Line<T> &l) {
    std::cerr << cross(l.a - l.b, l.a - p) << ' ' << length(l) << ' ' << cross(l.a - l.b, l.a - p) / length(l) << '\n';
...
__int128 cross(__int128 x1, __int128 y1, __int128 x2, __int128 y2) {
    return x1 * y2 - x2 * y1;
}
...
void solve() {
  ...
  auto dis2 = [&](int i, int j) {
    return __int128(a[i].x - a[j].x) * (a[i].x - a[j].x) + __int128(a[i].y - a[j].y) * (a[i].y - a[j].y);
  };
  ...
  auto bad = [&](int i, int j, int k) {
    ...
    // std::cout << distancePL(p, Line(a[i], a[j])) << '\n';
    // std::cout << distancePL(p, Line(a[i], a[k])) << '\n';
    // std::cout << cross(a[j] - p, a[j] - a[i]) << '\n';
    // std::cout << cross(a[k] - p, a[k] - a[i]) << '\n';
    __int128 d2 = dis2(i, k);
    __int128 s2 = d2 * r * r;
    __int128 c1 = abs(cross(p.x - a[i].x, p.y - a[i].y, p.x - a[k].x, p.y - a[k].y));
    c1 *= c1;
    // std::cout << c1 << '\n' << s2 << '\n';
    if (c1 < s2) return true;
    return cross(a[j] - p, a[j] - a[i]) * cross(a[k] - p, a[k] - a[i]) < 0;
  };
  // long double area = 0, ans = 0;
  int area = 0, ans = 0;
  for (int i = 0, j = 1; i < n; ++i, area -= abs(cross(a[j] - a[i - 1], a[j] - a[i % n]))) {
    while (!bad(i, j, (j + 1) % n)) {
      // std::cerr << i << ' ' << j + 1 << '\n';
      area += abs(cross(a[j] - a[i], a[(j + 1) % n] - a[i]));
      j = (j + 1) % n;
    }
