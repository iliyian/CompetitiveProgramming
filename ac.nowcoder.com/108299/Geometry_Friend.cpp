...
auto cross(std::pair<int, int> a, std::pair<int, int> b) {return a.first * b.second - a.second * b.first;}
auto dir(std::pair<int, int> a, std::pair<int, int> b) {return std::make_pair(b.first - a.first, b.second - a.second);}
...
void solve() {
  ...
  int mx = 0;
  for (int i = 0; i < n; i++) {
    dis[i] = __int128_t(a[i].first - p.first) * (a[i].first - p.first) + __int128_t(a[i].second - p.second) * (a[i].second - p.second);
    mx = std::max(mx, dis[i]);
  }
  std::vector<long double> v;
  for (int i = 0; i < n; i++) {
    if (mx == dis[i]) {
      v.push_back(atan2(a[i].second - p.second, a[i].first - p.first));
    }
  }
  std::ranges::sort(v);
