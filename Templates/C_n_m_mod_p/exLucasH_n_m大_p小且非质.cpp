#include <bits/stdc++.h> //exLucas#n,m�� pС�ҷ���
...
struct Excrt {
  ...
  {                    // ax+by=c if ok return 1/a/b else return 0
    ...
    a = slowmul(a, c, y);
    return 1; /*a*=c;b*=c;will overflow when max(a,b)^2>1e18)*/
  }
  pair<ll, ll> excrt(
      pair<ll, ll> pa,
      pair<ll, ll>
...
struct exLucas {
  ...
  vector<pair<ll, ll>> era(vector<ll> &v) {
    vector<pair<ll, ll>> v2;
    for (auto k : v) {
      if (v2.empty() || v2.back().first != k)
        v2.push_back({k, k});
      else
        v2.back().second *= k;
    }
  ...
  void precal(ll _mod) {
    mod = _mod;
    vector<ll> f2 = mil.disolve(mod);
    facs = era(f2);
    siz = facs.size();
    phi = mod;
    for (auto k : facs)
      phi = phi / k.first * (k.first - 1);
    vector<ll> bas(siz + 1, 0);
    bas[0] = 1;
    mul.resize(siz);
    for (int e = 0; e < siz; e++) {
      ll pk = facs[e].second, p = facs[e].first;
      mul[e].resize(pk + 1);
      mul[e][0] = 1;
      for (int i = 1; i < pk; i++)
        mul[e][i] = (i % p) ? mul[e][i - 1] * i % pk : mul[e][i - 1];
    }
  }
  pair<ll, ll> fac(ll n, int pos) {
    if (!n)
      return {1, 0};
    pair<ll, ll> r2 = fac(n / p, pos);
    ll b = n / p + r2.second;
    ll a = poww(mul[pos][pk - 1], n / pk, pk) * mul[pos][n % pk] % pk *
           r2.first % pk;
    return {a, b};
  }
  ll C(ll n, ll m) {
    ...
    for (int i = 0; i < siz; i++) {
      p = facs[i].first;
      pk = facs[i].second;
      pair<ll, ll> a = fac(n, i), b = fac(m, i), c = fac(n - m, i);
      ll val = a.first * invv(b.first, pk) % pk * invv(c.first, pk) % pk,
         cnt = a.second - b.second - c.second;
      (val *= poww(p, cnt, pk)) %= pk;
      ans = Exc.excrt(ans, {pk, val});
    }
    return ans.second;
  }
} exL;
